...
public class LichhocsController(QuanLyHocSinhContext context) : Controller
{
    ...
    // GET: Lichhocs
    public async Task<IActionResult> Index(int? lopFilter)
    {
        var viewModel = new ScheduleGridViewModel();

        // 1. Chuẩn bị dữ liệu cho dropdown bộ lọc Lớp
        var tatCaCacLop = await context.Lops.OrderBy(l => l.TenLop).ToListAsync();
        viewModel.LopOptions = tatCaCacLop.Select(l => new SelectListItem
        {
            Value = l.MaLop.ToString(),
            Text = l.TenLop
        }).ToList();

        // 2. Lấy dữ liệu Lịch học dựa trên bộ lọc
        var lichHocQuery = context.Lichhocs
        ...
        {
            lichHocQuery = lichHocQuery.Where(l => l.MaLop == lopFilter.Value);
            viewModel.SelectedLopId = lopFilter.Value;
            viewModel.TenLopHienThi = tatCaCacLop.FirstOrDefault(l => l.MaLop == lopFilter.Value)?.TenLop ?? "";
        }

        var lichhocs = await lichHocQuery.ToListAsync();

        // 3. Chuyển đổi dữ liệu danh sách thành dạng lưới (dictionary)
        foreach (var lich in lichhocs)
        {
            if (lich.ThuTrongTuan != null && lich.TietHoc.HasValue)
            {
                // Nếu chưa có key cho "Thứ", tạo mới
                ...
                // Gán lịch học vào đúng ô [Thứ][Tiết]
    ...
    public async Task<IActionResult> MySchedule(int? selectedHocKyId) // <<< Thêm tham số
    {
        ...
        // Lấy tất cả học kỳ để tạo dropdown
        var allHockies = await context.Hockies
            .Include(h => h.MaNamHocNavigation)
            .OrderByDescending(h => h.NgayBatDau)
            .ToListAsync();

        viewModel.HocKyOptions = allHockies.Select(h => new SelectListItem
        {
            Value = h.MaHk.ToString(),
            Text = $"{h.TenHk} ({h.MaNamHocNavigation?.TenNamHoc})"
        }).ToList();

        // Xác định học kỳ sẽ được hiển thị
        Hocky? activeHk = null;
        if (selectedHocKyId.HasValue)
        {
            // Nếu người dùng chọn một học kỳ, tìm nó
            activeHk = await context.Hockies.FindAsync(selectedHocKyId.Value);
        }
        else
        {
            // Nếu không, tìm học kỳ hiện tại theo ngày thực tế
            var today = DateTime.Now.Date;
            activeHk = await context.Hockies
                .OrderByDescending(h => h.NgayBatDau)
                .FirstOrDefaultAsync(h => h.NgayBatDau.Date <= today && h.NgayKetThuc.Date >= today);
        }

        // Fallback: Nếu không có học kỳ nào đang diễn ra, lấy học kỳ mới nhất
        if (activeHk == null)
        {
            activeHk = allHockies.FirstOrDefault();
        }


        if (activeHk != null)
        {
            viewModel.TenHocKy = activeHk.TenHk ?? "N/A";
            viewModel.SelectedHocKyId = activeHk.MaHk;

            var scheduleData = await context.Lichhocs
                .Where(l => l.MaGv == gvId && l.MaHk == activeHk.MaHk)
                .Include(l => l.MaLopNavigation)
                .Include(l => l.MaMonHocNavigation)
                .ToListAsync();
